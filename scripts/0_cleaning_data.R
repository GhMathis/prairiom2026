...
# PLANT ----
read_xlsx("data/plants/OTU_plant.xlsx") %>%
  filter(str_detect(Host_code, "CAM"),
         !str_detect(Host_code, "21_CAM_1[45]")) %>%
  replace(is.na(.),0) %>%
  select(-Richness) %>%
  select(Host_code, where(~is.numeric(.x) && sum(.x) != 0 )) -> cover_plant_quad

write.table(cover_plant_quad, "outputs/clean_data_to_analise/cover_plant_quad.txt",row.names = F)
...
write.table(abund_plant_grid, "outputs/clean_data_to_analise/abund_plant_grid.txt",row.names = F)

# VIRUS ----

read.delim2("data/viruses/S1_Viral_OTU.txt", header = T) %>%
  rename(Host_code = "X") %>%
  filter(str_detect( Host_code, "CAM"),
         !str_detect(Host_code, "21_CAM_1[45]")) %>%
  select(Host_code, where(~is.numeric(.x) && sum(.x) != 0 ))-> otu_virus_cam

write.table(otu_virus_cam, "outputs/clean_data_to_analise/OTU_virus_CAM.txt",row.names = F)

# Fungi ----

load("data/fungi/fungi_OTU_and_TAX.Rdata")
...
write.table(fungi_OTU_sp_grid_filter2, file = "outputs/clean_data_to_analise/OTU_fungi.txt")

str(fungi_OTU_sp_grid_filter2)
hist(log1p(apply(fungi_OTU_sp_grid_filter2, 1, sum)))  

### otu fungi table for sbm (remove all OTU with less than 10 reads) ----
...
write.table(fungi_OTU_sp_grid_filter3, file = "outputs/clean_data_to_analise/OTU_fungi_for_sbm.txt")
